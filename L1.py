...
def why_this_course_exists():
    ...
    response = query_gpt4o(
        prompt="Why teach a course on building language models from scratch? Answer in one sentence.")  # @inspect response
    ...
    image("https://upload.wikimedia.org/wikipedia/commons/thumb/c/cc/Industrialisation.jpg/440px-Industrialisation.jpg",
          width=400)

    text("GPT-4 supposedly has 1.8T parameters. "), article_link(
        "https://www.hpcwire.com/2024/03/19/the-generative-ai-future-is-now-nvidias-huang-says")
    text("GPT-4 supposedly cost $100M to train. "), article_link(
        "https://www.wired.com/story/openai-ceo-sam-altman-the-age-of-giant-ai-models-is-already-over/")
    text("xAI builds cluster with 200,000 H100s to train Grok. "), article_link(
    ...
    text("Stargate (OpenAI, NVIDIA, Oracle) invests $500B over 4 years. "), article_link(
    ...
    text("Exmaple 1: fraction of FLOPs spent in attention versus MLP changes with scale. "), tweet_link(
    ...
    link("https://arxiv.org/abs/2005.04305"), text(
...
def current_landscape():
    ...
    text(
        "- Open-weight models (e.g., DeepSeek): weights available, paper with architecture details, some training details, no data details "), link(
        deepseek_v3)
    text(
        "- Open-source models (e.g., OLMo): weights and data available, paper with most details (but not necessarily the rationale, failed experiments) "), link(
        olmo_7b)
...
def basics():
    ...
    text("- Lower-dimensional attention: group-query attention (GQA), multi-head latent attention (MLA) "), link(
        gqa), link(mla)
    ...
    text("## Assignment 1")
    link(title="[GitHub]", url="https://github.com/stanford-cs336/assignment1-basics"), link(title="[PDF]",
    ...
    text("- Leaderboard: minimize OpenWebText perplexity given 90 minutes on a H100 "), link(
...
def systems():
    ...
    image(
        "https://www.fibermall.com/blog/wp-content/uploads/2024/09/the-hardware-topology-of-a-typical-8xA100-GPU-host.png",
        width=500)
    ...
    text("## Assignment 2")
    link(title="[GitHub from 2024]", url="https://github.com/stanford-cs336/spring2024-assignment2-systems"), link(
        title="[PDF from 2024]",
...
def scaling_laws():
    ...
    text("## Assignment 3")
    link(title="[GitHub from 2024]", url="https://github.com/stanford-cs336/spring2024-assignment3-scaling"), link(
        title="[PDF from 2024]",
...
def data():
    ...
    text("- Might have to license data (e.g., Google with Reddit data) "), article_link(
    ...
    text("## Assignment 4")
    link(title="[GitHub from 2024]", url="https://github.com/stanford-cs336/spring2024-assignment4-data"), link(
        title="[PDF from 2024]",
...
def alignment():
    ...
    text("## Assignment 5")
    link(title="[GitHub from 2024]", url="https://github.com/stanford-cs336/spring2024-assignment5-alignment"), link(
        title="[PDF from 2024]",
...
def tokenization():
    text("This unit was inspired by Andrej Karpathy's video on tokenization; check it out! "), youtube_link(
...
def merge(indices: list[int], pair: tuple[int, int], new_index: int) -> list[
    int]:  # @inspect indices, @inspect pair, @inspect new_index
...
def tokenization_examples():
    text("To get a feel for how tokenizers work, play with this "), link(title="interactive site",
...
def character_tokenizer():
    ...
    text("There are approximately 150K Unicode characters. "), link(title="[Wikipedia]",
...
def bpe_tokenizer():
    ...
    text("The BPE algorithm was introduced by Philip Gage in 1994 for data compression. "), article_link(
